...
function check_java_version {
    if [[ "$_java" ]]; then
        ...
        if [[ "$version" < "1.8" ]]; then
            echo "[ERROR] ScioDB client requires JDK version 1.8 or later, the current version is $version "
            exit 1
        fi
    fi
}
...
java $JAVA_OPTS_SCIODB -cp $CLASSPATH org.sciodb.Client "$@"

#nohup java $JAVA_OPTS_SCIODB -cp $CLASSPATH org.sciodb.Client "$@" > logs/node.log 2>&1 &
#pid=$!
